    ...
    try {
      ...
      if (info && Number(info.bettingEndTime) <= Math.floor(Date.now() / 1000)) {
        try {
          ...
          // All clients MUST use the same seed for synchronization
          ...
          if (contractSeed) {
            ...
            // If seed not generated yet, try to generate it
            // This can be called by anyone - whoever calls it first wins
            if (!contractSeed.generated && signer) {
              console.log(`[Race ${currentRace}] Seed not generated yet, attempting to generate...`);
              try {
                const result = await generateRaceSeed(signer, currentRace);
                if (result.success) {
                  console.log(`[Race ${currentRace}] Seed generated successfully!`, result.txHash);
                  // Refetch seed after generation
                  setTimeout(async () => {
                  ...
                  }, 3000); // Wait 3 seconds for tx to be mined
                } else {
                  console.log(`[Race ${currentRace}] Failed to generate seed (might be already generated by someone else):`, result.error);
                }
              } catch (error) {
                // Silently fail - someone else might have generated it
                console.log(`[Race ${currentRace}] Seed generation attempt failed (normal if someone else generated it)`);
              }
            }
          } else {
